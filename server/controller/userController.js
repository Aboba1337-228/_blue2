...
class User {
    ...
    async login(req, res){
        try {
            const { numSchools, numClass, numSip } = req.body
            
            const connect = await connection;
            const [rows, fields] = await connect.execute('SELECT * FROM `users` WHERE `numSchools` = ? and `numClass` = ? and `numSip` = ?', [numSchools, numClass, numSip]);
    ...
    async register(req, res){
        try {
            const { numSchools, numClass, numSip, city, schools } = req.body  

            const connect = await connection;
            const numberS = Math.random(1,1000000000000000)
            await connection.execute("INSERT INTO `users` (`numSchools`, `numClass`, `numSip`, `number`, `city`, `schools`) VALUES (?, ?, ?, ?, ?, ?)", [numSchools, numClass, numSip, numberS, city, schools]);
